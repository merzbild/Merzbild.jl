@testset "convection 1D and boundaries" begin
    ...
    # 1D specularly reflecting boundaries with other BC type
    bc_list = (FullySpecularBC1D(),
               FullySpecularBC1D())
    ...
    # we sampled a lot of particles, reflected y velocity should be very close to prescribed
    @test abs((phys_props.v[2, 1, 1] - vy_left_wall) / vy_left_wall) < 3.75e-3

    # z-velocity should be almost zero
    @test abs(phys_props.v[3, 1, 1] - 0.0) < 3.7
    ...
    # we sampled a lot of particles, reflected y velocity should be very close to prescribed
    @test abs((phys_props.v[2, 100, 1] - vy_right_wall) / vy_right_wall) < 3.85e-3

    # z-velocity should be almost zero
    @test abs(phys_props.v[3, 100, 1] - 0.0) < 4.5
    ...
    convect_particles!(rng, grid, bc_list, particles[1], pia, 1, species_data, 1.0)
    sort_particles!(gridsorter, grid, particles[1], pia, 1)
    ...
    convect_particles_and_compute_cell!(rng, grid, bc_list, particles[1], pia, 1, species_data, 2.0)
    ...
    convect_particles_and_compute_cell!(rng, grid, bc_list, particles[1], pia, 1, species_data, 1.0)
    sort_particles!(gridsorter, particles[1], pia, 1)
