@testset "nnls_merging" begin
    ...
    @test abs(mnnls.minvx - 1.0) < eps()
    @test abs(mnnls.maxvx - 50.0) < eps()
    @test abs(mnnls.minvy - (5.0 - 2.0 * 50)) < eps()
    @test abs(mnnls.maxvy - (5.0 - 2.0 * 1)) < eps()
    @test abs(mnnls.minvz - (-100 + 0.25)) < eps()
    @test abs(mnnls.maxvz - (-100 + 0.25 * 2500)) < eps()

    compute_props!(phys_props, pia, particles, species_list)
    # test that merging conserves mass / momentum / energy
    @test phys_props.np[1, 1] < np0
    @test abs(n0 - phys_props.n[1, 1]) < 9e-15
    @test sum(abs.(v0 - phys_props.v[:, 1, 1])) < 2e-13
    @test abs(T0 - phys_props.T[1, 1]) < 3e-14

    # test higher-order moment in g
    @test abs(M40 - phys_props.moments[1, 1, 1]) < 4e-12
