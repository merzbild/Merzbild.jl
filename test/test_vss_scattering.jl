...
@testset "hard sphere scattering isotropy" begin
    particles_data_path = joinpath(@__DIR__, "..", "data", "particles.toml")
    hs_data_path = joinpath(@__DIR__, "..", "data", "hard_sphere.toml")

    species_data = load_species_data(particles_data_path, ["Ar", "He"])
    interaction_data = load_interaction_data(hs_data_path, species_data)
    interaction = interaction_data[1, 2]
    ...
    for _ in 1:n_samples
        Merzbild.scatter!(rng, HardSphere(), collision_data, interaction, p1, p2)
    ...
    for i in 1:3
        for j in 1:3
            @test abs(second_moments[i, j] - (i == j ? 1.0 / 3.0 : 0.0)) < 1e-2
        end
    end
end
