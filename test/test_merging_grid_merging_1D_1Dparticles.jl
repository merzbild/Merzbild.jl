@testset "grid-based merging 1D, 1D particles" begin
    ...
    # such that mean(x)+-std(x) is outside of the domain
        ...
        for i in 1:4
            ...
            vp[i] = Particle{1}(2.0, SVector{3,Float64}(0.5 - i^2, -3.0 + i, 4.0 + 0.3 * i), SVector{1,Float64}(x_cell1[i]))
        end

        x_cell2 = [0.55, 0.95, 0.85, 0.99]
        for i in 5:8
            Merzbild.update_particle_buffer_new_particle!(vp, i)
            vp[i] = Particle{1}(3.0, SVector{3,Float64}(0.5 + i^2, -3.0 + 2 * i, 4.0 - i), SVector{1,Float64}(x_cell2[i-4]))
        end
