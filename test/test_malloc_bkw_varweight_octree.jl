@testset "malloc: bkw variable weight + octree N:2 merging with particles with dim(x)=0,1,2,3" begin
    ...
    @testset "0D particles, 0D merging" begin
        ...
        for ts in 1:30
            ...
            if phys_props.np[1,1] > threshold
                bytes_merge = @allocated merge_octree!(rng, oc, particles[1], pia, 1, 1, Ntarget)
                merges = true
                @test bytes_merge == 0
            end
    ...
    @testset "1D particles, 1D merging" begin
        ...
        for ts in 1:3
            ...
            if phys_props.np[1,1] > threshold
                merge_octree!(rng, oc1, particles3[1], pia, 1, 1, Ntarget)
                merges = true
            end
        ...
        for ts in 1:20
            ...
            if phys_props.np[1,1] > threshold
                bytes_merge = @allocated merge_octree!(rng, oc1, particles3[1], pia, 1, 1, Ntarget)
                merges = true
                @test bytes_merge == 0
            end
    ...
    @testset "2D particles, 2D merging" begin
        ...
        for ts in 1:3
            ...
            if phys_props.np[1,1] > threshold
                merge_octree!(rng, oc2, particles3[1], pia, 1, 1, Ntarget)
                merges = true
            end
        ...
        for ts in 1:20
            ...
            if phys_props.np[1,1] > threshold
                bytes_merge = @allocated merge_octree!(rng, oc2, particles3[1], pia, 1, 1, Ntarget)
                merges = true
                @test bytes_merge == 0
            end
    ...
    @testset "3D particles, 3D merging" begin
        ...
        for ts in 1:3
            ...
            if phys_props.np[1,1] > threshold
                merge_octree!(rng, oc3, particles3[1], pia, 1, 1, Ntarget)
                merges = true
            end
        ...
        for ts in 1:20
            ...
            if phys_props.np[1,1] > threshold
                bytes_merge = @allocated merge_octree!(rng, oc3, particles3[1], pia, 1, 1, Ntarget)
                merges = true
                @test bytes_merge == 0
            end
