@testset "conservative N:1 octree_merging, 2D particles" begin
    ...
    function create_particle_in_octant(octant, v_val; w=1.0)
        ...
        return Particle{2}(Float64(w), SVector{3,Float64}(v_x, v_y, v_z), SVector{2,Float64}(1.0, -10.0))
    end
    ...
    octree = OctreeMerge{2,1}(OctreeBinMidSplit; init_bin_bounds=OctreeInitBinMinMaxVel)
    ...
    octree2 = OctreeMerge{2,1}(OctreeBinMidSplit; init_bin_bounds=OctreeInitBinMinMaxVel)
    ...
    # symmetric octree with split at v0 = (0.0, 0.0, 0.0)
    octree = OctreeMerge{2,1}(OctreeBinMidSplit; init_bin_bounds=OctreeInitBinMinMaxVel, bin_bounds_compute=OctreeBinBoundsInherit)
