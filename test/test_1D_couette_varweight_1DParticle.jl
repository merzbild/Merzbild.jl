...
@testset "couette test variable weight with Particle{1}" begin
    ...
    oc = OctreeN2Merge{1}(OctreeBinMidSplit; init_bin_bounds=OctreeInitBinMinMaxVel, max_Nbins=6000)
    ...
    @test maximum(abs.(ref_sol["kinetic_energy_flux"][:, 1, 1:5] .- sol["kinetic_energy_flux"][:, 1, 1:5])) < 6.0e-11
