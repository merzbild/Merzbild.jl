@testset "nnls_merging, 0D particles" begin

    function create_particles()
        pv = ParticleVector{0}(50)

        for i in 1:50
            Merzbild.update_particle_buffer_new_particle!(pv, i)
            pv.particles[i] = Particle{0}(1.0, SVector{3,Float64}(i, 5 - 2.0 * i, -100.0 + 0.25 * i^2), SVector{0,Float64}())
        end
    ...
    function create_particles2(; mult=0.0, mult2=1.0)
        pv = ParticleVector{0}(16)

        pv.particles = [Particle{0}(1.0, SVector{3,Float64}(1.0, 1.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, 1.0, -1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, -1.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, -1.0, -1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-1.0, 1.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-1.0, 1.0, -1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-1.0, -1.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-1.0, -1.0, -1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(2.0, 2.0, 2.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(2.0, 2.0, -2.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(2.0, -2.0, 2.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(2.0, -2.0, -2.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-2.0, 2.0, 2.0), SVector{0,Float64}()),
                        Particle{0}(1.0*mult2, SVector{3,Float64}(-2.0, 2.0, -2.0), SVector{0,Float64}()),
                        Particle{0}(1.0*mult2, SVector{3,Float64}(-2.0, -2.0, 2.0), SVector{0,Float64}()),
                        Particle{0}(1.0*mult2, SVector{3,Float64}(-2.0, -2.0, -2.0), SVector{0,Float64}())]
    ...
    function create_particles3()
        pv = ParticleVector{0}(6)

        pv.particles = [Particle{0}(1.0, SVector{3,Float64}(3.0, 1.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, 4.0, -1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, -8.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(1.0, -1.0, -9.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-7.0, -11.0, 1.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-9.0, 6.0, -1.0), SVector{0,Float64}())]
    ...
    function create_particles4()
        pv = ParticleVector{0}(2)

        pv.particles = [Particle{0}(1.0, SVector{3,Float64}(3.0, 1.0, -4.0), SVector{0,Float64}()),
                        Particle{0}(1.0, SVector{3,Float64}(-3.0, 1.0, 4.0), SVector{0,Float64}()),]
    ...
    # copy-pasted from 3d case, we don't conserve any spatial moments, but let's check that they
    # are neglected correctly
    particles = [create_particles2(mult=2.0,mult2=2.0)]
    
    pos_moments = [[0,0,1],[1,0,0],[0,1,0]]

    mnnls_pos = NNLSMerge{0}([], 30; multi_index_moments_pos=pos_moments)
    ...
    particles = [create_particles2(mult=2.0,mult2=2.0)]
    pia = ParticleIndexerArray(length(particles[1]))
    result = merge_nnls_based!(rng, mnnls_pos, particles[1], pia, 1, 1; w_threshold=0.25)
    ...
    mnnls_pos = NNLSMerge{0}(add_vel_moments, 30; multi_index_moments_pos=pos_moments)
    mnnls_no_pos = NNLSMerge{0}(add_vel_moments, 30)
    @test mnnls_pos.n_total_conserved == mnnls_pos.n_moments_vel + mnnls_pos.n_moments_pos
    ...
    @test mnnls_pos.n_moments_pos == 0
    @test mnnls_pos.pos_i_x == -1
    @test mnnls_pos.pos_i_y == -1
    @test mnnls_pos.pos_i_z == -1
    @test length(mnnls_pos.rhs_vector) == length(mnnls_no_pos.rhs_vector) == 9
    ...
    mnnls_pos = NNLSMerge{0}(add_vel_moments, 25; multi_index_moments_pos=pos_moments, matrix_ncol_nprealloc=10)
    ...
    @test size(mnnls_pos.vel_pos_matrix_scratch) == (3+0, 36)
    @test length(mnnls_pos.column_norms_scratch) == 36

    for i in 25:35
        @test size(mnnls_pos.vel_pos_matrices[i-25+1]) == (3+0, i)
    ...
    mnnls = NNLSMerge{0}(add_vel_moments, length(particles[1]); multi_index_moments_pos=[])
