@testset "grid-based merging 1D, 2D particles" begin
    
    function create_particles_in_2cells()
    # create 4 particles in 2 cells
    # such that mean(x)+-std(x) is outside of the domain
        vp = ParticleVector{2}(8)

        x_cell1 = [0.05, 0.05, 0.05, 0.45]
        for i in 1:4
            Merzbild.update_particle_buffer_new_particle!(vp, i)
            vp[i] = Particle{2}(2.0, SVector{3,Float64}(0.5 - i^2, -3.0 + i, 4.0 + 0.3 * i), SVector{2,Float64}(x_cell1[i], 0.0))
        end

        x_cell2 = [0.55, 0.95, 0.85, 0.99]
        for i in 5:8
            Merzbild.update_particle_buffer_new_particle!(vp, i)
            vp[i] = Particle{2}(3.0, SVector{3,Float64}(0.5 + i^2, -3.0 + 2 * i, 4.0 - i), SVector{2,Float64}(x_cell2[i-4], 0.0))
        end
