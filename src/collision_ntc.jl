...
function estimate_sigma_g_w_max_ntc_n_e!(species_n, species_e, cell, rng, collision_factors, pia,
    collision_data, interaction, n_e_interactions, n_e_cs, particles_n, particles_e,
    Δt, V; min_coll=5, n_loops=3)
    # compute ncoll
    # loop over particles
    # update sigma_g_w_max
    # collide
    ...
    for _ in 1:n_loops
        ...
        for _ in 1:n_coll_int
            ...
            if (collision_data.g > eps())
                collision_data.E_coll_eV = compute_cross_sections!(n_e_cs, interaction, collision_data.g, n_e_interactions, species_n)
...
function ntc_n_e!(species_n, species_e, species_ion, cell, rng, collision_factors, pia,
    collision_data, interaction, n_e_interactions, n_e_cs, particles_n, particles_e, particles_ion,
    Δt, V)
    # no event splitting
    # compute ncoll
    # loop over particles
    ...
    for _ in 1:n_coll_int
        ...
        if (collision_data.g > eps())
            ...
            collision_data.E_coll_electron_eV = compute_cross_sections!(n_e_cs, interaction, collision_data.g, n_e_interactions, species_n)
            sigma_g_w_max = get_cs_total(n_e_cs, species_n, n_e_interactions) * collision_data.g * max(particles_n[i].w, particles_e[k].w)
            ...
            if (rand(rng, Float64) < sigma_g_w_max / collision_factors.sigma_g_w_max)
                collision_factors.n_coll_performed += 1
                compute_com!(collision_data, interaction, particles_n[i], particles_e[k])
                ...
                if (particles_n[i].w > particles_e[k].w)
                    ...
                    # first need to update the particle indexer struct
                    update_particle_indexer_new_particle(species_n, cell, pia)
                ...
                elseif (particles_n[i].w == particles_e[k].w)
                    ...
                    update_particle_indexer_new_particle(species_e, cell, pia)
                ...
                if (R < n_e_cs[species_n].prob_vec[1])  # TODO: fix indexing! 
                    # elastic collision
                    scatter_vhs(rng, collision_data, interaction, particles_n[i], particles_e[k])
                else
                    ...
                    update_particle_indexer_new_particle(species_ion, cell, pia)
                    ...
                    compute_g_new_ionization(collision_data, interaction,
...
function ntc_n_e_es!(species_n, species_e, species_ion, cell, rng, collision_factors, pia,
    collision_data, interaction, n_e_interactions, n_e_cs, particles_n, particles_e, particles_ion,
    Δt, V)
    # no event splitting
    # compute ncoll
