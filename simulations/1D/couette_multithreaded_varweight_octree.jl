...
function run(seed, T_wall, v_wall, L, ndens, nx, ppc_sampled, merge_threshold, merge_target, Δt, output_freq, n_timesteps, avg_start; chunk_count_multiplier=1,
    ...
    @timeit "merge(t=0) + props compute" @threads for chunk_id in 1:n_chunks
    ...
    for t in 1:n_timesteps
        ...
        @timeit "collide+merge+squash+convect+sort" @threads for chunk_id in 1:n_chunks
            for cell in cell_chunks[chunk_id]
                ...
                @inbounds if pia_chunks[chunk_id].indexer[cell,1].n_local > merge_threshold
                    merge_octree_N2_based!(rng_chunks[chunk_id], oc_chunks[chunk_id], particles_chunks[chunk_id][1], pia_chunks[chunk_id], cell, 1, merge_target, grid)
                    squash_pia!(particles_chunks[chunk_id], pia_chunks[chunk_id])
                end
            end
        ...
        if (t % output_freq == 0)
            ...
            # println("ndens = $(sum(phys_props.n))")  # just to make sure particles are not being lost, etc.
        end
