...
function run(seed, E_Tn, n_t, n_up_to_total, n_full_up_to_total, threshold_electrons, np_target_electrons_octree; adds=0)
    ...
    # fname =  "../../Data/PIC_DSMC/NNLS_merging/0D_ionization/Ar_e/NNLS_rate_preserving_macc_es/"
    fname =  "../../Data/PIC_DSMC/NNLS_merging/0D_ionization/Ar_e/NNLS_macc_es/"
    ...
    for ts in 1:n_t
        ...
        if pia.n_total[3] > threshold_electrons
            if firstm
                ...
                @timeit "NNLSmerge" nnls_success_flag = merge_nnls_based!(rng, mnnls, vref, particles, 1, 3, pia, 0)
                firstm = false
            else
