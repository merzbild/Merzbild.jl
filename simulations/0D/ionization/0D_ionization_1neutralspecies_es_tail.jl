...
This also writes data for the tail functions.
...
function run(seed, E_Tn, n_t, threshold_electrons, np_target_electrons,
    ...
    E_ionization = Merzbild.get_ionization_threshold(n_e_interactions, index_neutral)
    tail_energies = tail_cutoff_energies(E_ionization)
    println("Ionization threshold: $(E_ionization) eV; tail cutoffs: $(tail_energies) eV")
    tail_rec = TailRecorder(tail_energies, species_data[index_electron].mass, n_t)
    ...
    for ts in 1:n_t
        ...
        @timeit "tail pre" record_tail_pre!(tail_rec, ts, particles[3], pia, 1, 3, collision_factors[s1,s2,1])
        merge_kind = TAIL_NO_MERGE

        if pia.n_total[3] > threshold_electrons
            @timeit "merge e" merge_octree!(rng, oc_electrons, particles[3], pia, 1, 3, np_target_electrons)
            merge_kind = TAIL_MERGE_PRIMARY
        end

        @timeit "tail post" record_tail_post!(tail_rec, ts, particles[3], pia, 1, 3, merge_kind)
    ...
    print_timer()
    close_netcdf(ds)
    write_tail_netcdf(replace(fname, ".nc" => "_tail.nc"), tail_rec)
end
...
# for do_event_splitting in [true]  # try out different collision schemes
...
params = [[41, 38, 1], [62, 58, 1], [95, 88, 1], [131, 122, 1], [178, 166, 1], [236, 220, 1]] # 1.075
params = [[24000, 12000, 8], [41, 38, 16], [62, 58, 16], [95, 88, 16], [131, 122, 16], [178, 166, 16], [236, 220, 16]] # 1.075

 # we need more timesteps for the weaker field
for (n_t, external_E_field_Tn) in zip([500000], [400.0])
...
#### reference values run
# for sadd in 17:32
#     run(1234, external_E_field_Tn, n_t, 24000, 12000, cs_n_e_filepath; merging_bin_split=OctreeBinMidSplit, adds=sadd, do_es=true)
# end