...
In order to obtain variable-weight particles, one can either

* Use [`sample_particles_equal_weight!`](@ref) and perform merging afterwards --- this will lead to unequal weights
* Use the  [`sample_on_grid!`](@ref) function and its more specific version [`sample_maxwellian_on_grid!`](@ref), as mentioned
    in the [Overview of basic building blocks](@ref) section --- this will immediately lead to unequal particle weights
* Use [`sample_particles_phase_box_weighted!`](@ref) --- this will immediately lead to unequal particle weights

The grid-based sampling approaches compute the values of a VDF function on a discrete velocity grid and then re-interpret the grid points
...
**Important**: when estimating collision factors, one might need an estimate for the average weight of a particle;
this needs to be computed after the merging is performed on the initial simulation, otherwise the estimates might be significantly off
if merging reduces the number of particles by a large amount.
...
# estimate average Fnum - we need the post-merge value!
Fnum = ndens/Ntarget
